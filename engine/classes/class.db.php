...
class DBParser {
	...
	public function openConnection(){
		...
		if (is_null($this->dbh)) {
            $this->dbh = new mysqli($config["config"]["db"]["host"], $config["config"]["db"]["user"], $config["config"]["db"]["passwd"], $config["config"]["db"]["name"]);
			
            if (mysqli_connect_errno()) {
                $this->dbh = null;
				error("Error connect: ".mysqli_connect_error());
				
            } else {
                mysqli_report(MYSQLI_REPORT_ERROR);
            }
			
			if ($config["config"]["db"]["charset"] != '') { 
				$this->dbh->query("SET NAMES ".$config["config"]["db"]["charset"]."");
			}
        }
        return $this->dbh;
	}
	...
	public function getTableName($tbl) {
		$config = $this->config;
        return ".`" . $config["config"]["db"]["prefix"] . $tbl . "`";
    }
