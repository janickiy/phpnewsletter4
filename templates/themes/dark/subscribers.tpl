...
<ul class="BtnPanelIcon">
  <li><a title="${PROMPT_ADD_USER}" href="./?task=add_user"> <span class="fa fa-user-plus fa-2x"></span> <span class="IconText">${STR_ADD_USER}</span> </a> </li>
  <li><a title="${PROMPT_REMOVE_SUBSCRIBERS}" onclick="return confirm('${ALERT_CLEAR_ALL}');" href="./?task=subscribers&remove=all"> <span class="fa fa-trash-o fa-2x"></span> <span class="IconText">${STR_REMOVE_ALL_SUBSCRIBERS}</span> </a> </li>
  <li><a title="${PROMPT_IMPORT_SUBSCRIBERS}" href="./?task=import"> <span class="fa fa-download fa-2x"></span> <span class="IconText">${STR_IMPORT_USER}</span> </a> </li>
  <li><a title="${PROMPT_EXPORT_SUBSCRIBERS}" href="./?task=export"> <span class="fa fa-upload fa-2x"></span> <span class="IconText">${STR_EXPORT_USER}</span> </a> </li>
</ul>
  ...
  <table class="datatable table table-striped table-bordered table-hover" border="0" cellspacing="0" cellpadding="0" width="100%">
    <thead>
      <tr>
        ...
        <th width="30%" class="${TH_CLASS_NAME}"><a href="./?task=subscribers&name=${GET_NAME}<!-- IF '${SEARCH}' != '' -->&search=${SEARCH}<!-- END IF -->${PAGENAV}">${TABLE_NAME}</a></th>
        <th width="30%" class="${TH_CLASS_EMAIL}"><a href="./?task=subscribers&email=${GET_EMAIL}${PAGENAV}<!-- IF '${SEARCH}' != '' -->&search=${SEARCH}<!-- END IF -->">${TABLE_EMAIL}</a></th>
    ...
    <tbody>
      ...
      <tr class="td-middle<!-- IF '${STATUS_CLASS}' == 'noactive' --> danger<!-- END IF -->">
        ...
        <td>${STR_STAT}</td>
        <td class="center"><a class="btn btn-primary" href="./?task=edit_user&id_user=${ID_USER}" title="${STR_EDIT}"> <i class="icon icon-edit"></i></a> <a class="btn btn-danger" href="./?task=subscribers&remove=${ID_USER}" title="${STR_REMOVE}"> <i class="icon icon-trash"></i></a></td>
      </tr>
      <!-- END column -->
    </tbody>
  </table>
